...
1. **`analyze --path DIR`** — read-only OBSERVED import graph. `--lang python` (default) uses Python AST; JS/TS/Go/Rust/Ruby use AST-grade token parsers; or `auto`. Writes `analysis.json` + diagram bundle when `--out` is set.
