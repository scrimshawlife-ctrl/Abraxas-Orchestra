...
3. **`optimize --from analysis.json`** — plan only. Does **not** rewrite the analyzed tree. `--apply` is refused in 0.2.x (Phase C deferred).
...
Local stdlib CLI. No network I/O in structure/analyze/optimize-plan paths. Analyze writes only under `--out`. Optimize apply is not enabled in 0.2.x.
